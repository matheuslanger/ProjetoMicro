...
//---------------------------------------------------------------------------------------------
unsigned char escreve_LCD(char *c)
{
...
//---------------------------------------------------------------------------------------------
//Presentinho do Gregory(2020/1), printa numeros de 0 até 65536 e retorna a posição final do cursor
//---------------------------------------------------------------------------------------------
char print_LCD(unsigned int num, char pos){
	int i=0;
	int j=0;
	//Se o número for diferente de 0
	if(num!=0){
		...
		ident_num(num,digitos);//transforma o número em vetor
		//Indica a posição para o primeiro digito
		cmd_LCD(pos,0);
		//Escreve os digitos
		for(j=i;i>0;i--)
		cmd_LCD(digitos[i-1],1);
	}
	else if(!num){
		cmd_LCD(pos,0);
		cmd_LCD(48,1);
	...
	//Retorna a posição para o próximo número
	return pos+j+1;
}